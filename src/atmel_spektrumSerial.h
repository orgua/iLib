...
static const uint8_t  SPEKTRUM_CHAN_STATIC =
        (4); // first # Channels get transmitted every time, the others on change
  #else      // 10 bit frames
...
void spektrum_send(uint16_t *channel_data)
{
    ...
    for (uint8_t i = 0; i < 7; i++)
    {
        ...
        else
        {
            ...
            {
                if (position_start !=
                    position_newVal) // run through the remaining channels and find changed ones
                {
                    if (old_val[position_newVal - SPEKTRUM_CHAN_STATIC] !=
                        (channel_data[position_newVal] & SPEKTRUM_VAL_MASK))
                    {
                        old_val[position_newVal - SPEKTRUM_CHAN_STATIC] =
                                (channel_data[position_newVal] & SPEKTRUM_VAL_MASK);
                        temp_i = position_newVal;
                        bitSet(bitmap_selected, position_newVal);
                    }
                    if (++position_newVal >= SPEKTRUM_CHAN_COUNT)
                        position_newVal = SPEKTRUM_CHAN_STATIC;
                }
                else // fill the remaining places with unchanged channels
                {
                    ...
                    if (++position_sameVal >= SPEKTRUM_CHAN_COUNT)
                        position_sameVal = SPEKTRUM_CHAN_STATIC;
                }
            }
        }
  #else
        ...
        spectrum_frame[2 + (i * 2)] =
                ((temp_i & SPEKTRUM_CHAN_MASK) << SPEKTRUM_CHAN_SHIFT) | (temp_val >> 8);
