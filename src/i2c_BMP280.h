...
private:
    ...
    static const uint8_t MSK_STATUS_IMUPDATE =
            (1 << 0); // 1 when NVM data is copied to image registers
...
public:
    BMP280(void)
        : dig_T1(0), dig_P1(0), dig_T2(0), dig_T3(0), dig_P2(0), dig_P3(0), dig_P4(0), dig_P5(0),
          dig_P6(0), dig_P7(0), dig_P8(0), dig_P9(0), tFine(0) {};
    ...
    inline uint8_t getEnabled(void) { return (3 & i2c.readByte(I2C_ADDRESS, REG_CTRL_MEAS)); };

    /**< do a software reset */
    inline void    reset(void) { i2c.writeByte(I2C_ADDRESS, REG_RESET, VAL_RESET); };
    ...
    void getMeasurement(float &pascal) { getPressure(pascal); };
    ...
    void getTemperature(int32_t &millicelsius)
    {
        ...
        var2  = (((((adc >> 4) - ((int32_t) dig_T1)) * ((adc >> 4) - ((int32_t) dig_T1))) >> 12) *
                 ((int32_t) dig_T3)) >>
                14;
