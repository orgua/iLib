...
Driver for the MMA8451-Sensor
 ...
 CONSUMPTION: standby X �A, measure X �A
...
private:
    ...
    static const uint8_t REG_HP_FILTER_CUTOFF =
            (0x0f);                                //(R/W) cut-off frequency is set to 16Hz @ 800Hz
    ...
    static const uint8_t REG_P_L_THS_REG =
            (0x14); //(R/W) Portrait to Landscape trip angle is 29 degree
    static const uint8_t REG_FF_MT_CFG =
            (0x15); //(R/W) Freefall/motion functional block configuration
    ...
    static const uint8_t REG_TRANSIENT_CFG =
            (0x1d); //(R/W) Transient functional block configuration
...
public:
    ...
    inline void    reset(void) { i2c.setRegister(I2C_ADDRESS, REG_CTRL_REG2, B01000000, 255); };

    inline uint8_t initialize() { return initialize(100, 8); };
...
#else
        i2c.setRegister(I2C_ADDRESS, REG_CTRL_REG1, 0x02,
                        0); // disable Fast Read Mode (FullResolution)
#endif
        setEnabled(1);
