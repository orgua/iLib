...
private:
    ...
    long xnode
            [9]; // = {0,INTERVAL/96,INTERVAL/48,INTERVAL/24,INTERVAL/12,INTERVAL/6,INTERVAL/3,INTERVAL/1.5,INTERVAL};
    long zero_setpoint;
...
public:
    ...
    void init(uint16_t interval, float pwr)
    {
        ...
        for (uint8_t p = 0; p < 8; ++p)
        {
            ...
            yy    = (float) ((pow(base * float(xnode[p + 1]), pwr)) -
                             (pow(base * float(xnode[p]), pwr)));
    ...
    void     set_zeropoint(long new_setpoint) { zero_setpoint = new_setpoint; };
