...
private:
    float MAX17047_SENSE{
            0.01}; // Application-specific sense resistor. Refer to MAX17047 documentation.
    ...
    static constexpr uint8_t MAX17047_AGE{
            0x07}; // FULL_CAP divided by DESIGN_CAP * 100%. Also known as state of health.
    ...
    static constexpr uint8_t MAX17047_SOC_AV{
            0x0E}; // State of Charge in % (Highbyte) considering all information
    static constexpr uint8_t MAX17047_REM_CAP_MIX{
            0x0F};                                    // Capacity in uVh (div Senseresistor for mAh)
    ...
    static constexpr uint8_t MAX17047_SOC_VF{
            0xFF}; // State of Charge according to voltage fuel gauge

    MAX17047(const MAX17047 &); // declaration only for copy constructor
    MAX17047 &
    operator=(const MAX17047 &); // declaration only for copy assignment --> make it uncopyable
...
public:
    ...
    float getSense(void) { return (MAX17047_SENSE); }

    // set the instance's sense resistor value
    void  setSense(const float &fvalue) { MAX17047_SENSE = fvalue; }
    /*
