...
private:
    ...
    inline const uint8_t HZ_TO_DIV(const uint16_t hz) { return ((1000 / hz) - 1); };
...
public:
    ...
    {
        if (gScaleRange < 4)
            i2c.setRegister(I2C_ADDRESS, REG_GYRO_CONFIG, MSK_GYRO_FS_SEL, gScaleRange << 3);
    ...
    {
        if (aScaleRange < 4)
            i2c.setRegister(I2C_ADDRESS, REG_ACCEL_CONFIG, MSK_ACCEL_FS_SEL, aScaleRange << 3);
    ...
    inline void reset(void)
    {
        ...
        i2c.setRegister(I2C_ADDRESS, REG_SIGNAL_PATH_RESET, rVector,
                        rVector); // Registers of Sensors
        i2c.setRegister(I2C_ADDRESS, REG_USER_CTRL, MSK_SIG_COND_RST,
                        MSK_SIG_COND_RST); // Signal paths of Sensors
        i2c.setRegister(I2C_ADDRESS, REG_PWR_MGMT_1, MSK_H_RESET,
                        MSK_H_RESET); // internal Registers
    };
    ...
    inline uint8_t initialize(void)
    {
        ...
        // Clocksource
        i2c.setRegister(I2C_ADDRESS, REG_PWR_MGMT_1, MSK_CLKSEL,
                        1); // should be VAL_CLOCK_PLL_XGYRO, is it?
        // PWR MGMT
        i2c.setRegister(I2C_ADDRESS, REG_PWR_MGMT_1,
                        MSK_CYCLE | MSK_GYRO_STANDBY_CYCLE | MSK_PD_PTAT, 0); // Normal operation

        // INTs
        i2c.setRegister(I2C_ADDRESS, REG_INT_PIN_CFG, MSK_INT_ANYRD_2CLEAR,
                        MSK_INT_ANYRD_2CLEAR); // Clear INT at read
        i2c.setRegister(I2C_ADDRESS, REG_INT_ENABLE, MSK_RAW_RDY_EN,
                        MSK_RAW_RDY_EN); // INT: Raw Data Ready to read

        // I2C-Master
        i2c.setRegister(I2C_ADDRESS, REG_USER_CTRL, MSK_I2C_MST_EN, 0); // disable I2C-Master
        i2c.setRegister(I2C_ADDRESS, REG_INT_PIN_CFG, MSK_BYPASS_EN,
                        MSK_BYPASS_EN); // enable Bypass mode!
    ...
    void getMeasurement(float xyz_AccTemGyr[])
    {
        ...
        // Scale
        xyz_AccTemGyr[3] =
                xyz_AccTemGyr[3] / 333.87 +
                21; // TEMP_degC = ((TEMP_OUT – RoomTemp_Offset)/Temp_Sensitivity) + 21degC
