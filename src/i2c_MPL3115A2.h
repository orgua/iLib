...
public:
    ...
    inline uint8_t getEnabled() { return (1 & i2c.readByte(I2C_ADDRESS, REG_SYSMOD)); };

    /**< do a software reset */
    inline void    reset() { i2c.writeByte(I2C_ADDRESS, REG_CTRL1, MSK_RST); };
    ...
    inline void setEventFlags(const uint8_t flags = (MSK_DATA_READY | MSK_PRES_READY |
                                                     MSK_TEMP_READY))
    {
        i2c.writeByte(I2C_ADDRESS, REG_PT_DATA_CFG,
                      (flags & 0x07)); // Enable all three pressure and temp event flags
    };
    ...
    inline void getMeasurement(float &pascal) { getPressure(pascal); };
