    ...
    while (1)
    {
        ...
        for (uint8_t x = 0; x < LEDS_SIZE; x++)
        {
            ...
            if (x < LEDS_DIST)
            {
                blue  = 0;
                red   = red - LEDS_INCR;
                green = green + LEDS_INCR;
            }
            else if (x < 2 * LEDS_DIST)
            {
                red   = 0;
                green = green - LEDS_INCR;
                blue  = blue + LEDS_INCR;
...
#define CLOCK_WIDTH     (1)
        ...
        {
            led_sel = led_mid + (LEDS_SIZE - CLOCK_WIDTH);
            analog.tv_hour += SCALEMAX;
        }
        ...
        for (uint8_t x = 0; x <= 2 * CLOCK_WIDTH; x++)
        {
            ...
            {
                value = (analog.tv_min - analog_LED);
            }
            if (value > CLOCK_INTENSITY) { value = 0; }
            else
            {
        ...
        for (uint8_t x = 0; x <= 2 * CLOCK_WIDTH; x++)
        {
            ...
            {
                value = (analog.tv_sec - analog_LED);
            }
            if (value > CLOCK_INTENSITY) { value = 0; }
            else
            {
